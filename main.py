...
def draw_surface(link, WalkLeft, WalkRight, walkCount, left, right):
    WIN.fill((WHITE))
    WIN.blit(TEMPLE, (0, 0))
    ...
    if walkCount + 1 >= 27:
        walkCount = 0
    if left:
        WIN.blit(WalkLeft[walkCount//3], (link.x, link.y))
        walkCount += 1
    elif right:
        WIN.blit(WalkRight[walkCount//3], (link.x, link.y))
        walkCount += 1
...
def link_movement(keys_pressed, link, jumpCount, isJump, walkCount, left, right):
...
def mainloop():
    ...
    while run:
        ...
        link_movement(keys_pressed, link, isJump, jumpCount, walkCount, left, right)
        draw_surface(link, WalkLeft, WalkRight, walkCount, left, right)
